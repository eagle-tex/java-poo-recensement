...
public class RechercheRegionsPlusPeuplees extends MenuService {
  ...
  public void traiter(Recensement recensement, Scanner scanner) throws ExceptionApplication {
    ...
    if (NumberUtils.toInt(nbRegionsStr) == 0) {
      throw new NombreNonEntierException("Le nombre de régions saisi doit être entier");
    }

    int nbRegions = Integer.parseInt(nbRegionsStr);
    if (nbRegions <= 0) {
      throw new DonneeInvalideException("Le nombre de régions doit être supérieur à 0");
    }
    ...
    for (int i = 0; i < nbRegions; i++) {
      ...
      System.out.println(
          "Region " + region.getNom() + " : " + region.getPopulation() + " habitants.");
    }
  }
}
